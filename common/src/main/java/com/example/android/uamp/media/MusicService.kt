...
open class MusicService : MediaBrowserServiceCompat() {
    ...
    private lateinit var mediaSources: MutableMap<String, MusicSource>
    ...
    override fun onCreate() {
        ...
        mediaSources = mutableMapOf<String, MusicSource>()

        val eventListener : PlayerEventListener = PlayerEventListener(applicationContext)
        exoPlayer.addListener(eventListener)
        initializeMeteoSource()
        initializeMusicSource()

        packageValidator = PackageValidator(this, R.xml.allowed_media_browser_callers)
    }

    private fun initializeMusicSource() {
        var musicSource = JsonSource(context = this, source = musicApiUri)

        //Coroutine to load the parse the json in multitask
        serviceScope.launch {
            musicSource.load()
        }
        mediaSources[MUSIC] = musicSource
    }

    private fun initializeMeteoSource() {
        fusedLocationClient = LocationServices.getFusedLocationProviderClient(this)

        var latitude: Double = 41.9109
        var longitude: Double = 12.4818


        fusedLocationClient.lastLocation
                .addOnFailureListener {
                    Log.v(TAG, "FAILURE to get Last known position")
                ...
                .addOnSuccessListener { location: Location? ->
                    ...
                    Log.v(TAG, "LATITUDINE  EEEEEEE" + location?.latitude)
                    //Toast.makeText(applicationContext, "Succes latitude : " + latitude + " longitude: " + longitude, Toast.LENGTH_SHORT).show()
                    val openWeatherApiUri: Uri = Uri.parse(OPENWEATHER_FORECAST_API_URL)
                            .buildUpon()
                            //      .path("search")
                            ...
                            .appendQueryParameter("units", SISTEMA)
                            .build();

                    var meteoSource = JsonSource(context = this, source = openWeatherApiUri)
                    serviceScope.launch {
                        meteoSource.load()
                    }
                    mediaSources[METEO] = meteoSource
    ...
    override fun onGetRoot(
        ...
        val lat = "lat", latitude.toString())
        val lon ="lon", longitude.toString())
        val appid = "appid", API_KEY)
        val lang="lang", LANGUAGE)
        val unit ="units", SISTEMA)
    ...
    override fun onLoadChildren(
        ...
        if (parentMediaId == UAMP_BROWSABLE_ROOT) {
            ...
            var mediaSource = mediaSources[MUSIC]!!
            if (parentMediaId == UAMP_METEO_ROOT &&
                    ContextCompat.checkSelfPermission(this,
                            Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                mediaSource = mediaSources[METEO]!!
            }
            val resultsSent = mediaSource.whenReady { successfullyInitialized ->
                if (successfullyInitialized) {
                    if (!browseTree.isInizialized(parentMediaId)) // TODO da levare e far si on click si refresha la pagina
                        browseTree.populateMediaItemsFromSource(mediaSource, parentMediaId)
                    val children = browseTree[parentMediaId]?.map { item ->
                        MediaItem(item.description, item.flag)
                    }
                    result.sendResult(children)
    ...
    override fun onSearch(
        ...
        val resultsSent = mediaSources[METEO]!!.whenReady { successfullyInitialized ->
            if (successfullyInitialized) {
                var resultsList = mediaSources[MUSIC]!!.search(query, extras ?: Bundle.EMPTY)
                        .map { mediaMetadata ->
                            MediaItem(mediaMetadata.description, mediaMetadata.flag)
                        }
                resultsList += mediaSources[METEO]!!.search(query, extras ?: Bundle.EMPTY)
                        .map { mediaMetadata ->
                            MediaItem(mediaMetadata.description, mediaMetadata.flag)
                        }
...
private const val MUSIC_URL = "https://storage.googleapis.com/uamp/catalog.json"

private const val OPENWEATHER_CURRENT_API_URL = "https://api.openweathermap.org/data/2.5/weather"
private const val OPENWEATHER_FORECAST_API_URL = "https://api.openweathermap.org/data/2.5/forecast"
private const val LANGUAGE = "it"
private const val SISTEMA = "metric"
private const val METEO = "meteo"
private const val MUSIC = "music"
//private val openWeatherApiUrl = OPENWEATHER_API_URL+"?lat={lat}&lon={lon}&appid={your api key}"
