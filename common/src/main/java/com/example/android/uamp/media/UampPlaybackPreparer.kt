...
class UampPlaybackPreparer(
        private val mediaSources: Map<String, MusicSource>,
    ...
    override fun onPrepareFromMediaId(mediaId: String?, extras: Bundle?) {
        mediaSources[METEO]!!.whenReady {

            var mediaItemToPlay: MediaMetadataCompat? = mediaSources[MUSIC]!!.find { item ->
                item.id == mediaId
            }
            if (mediaItemToPlay == null) {
                mediaItemToPlay = mediaSources[METEO]!!.find { item ->
                    item.id == mediaId
                }
            }
            if (mediaItemToPlay == null) {
                ...
                var metadataList = buildPlaylist(mediaItemToPlay, MUSIC)
                if (metadataList.isEmpty())
                    metadataList = listOf(mediaItemToPlay)
                //metadataList = buildPlaylist(mediaItemToPlay, METEO) // don't want reproduce always the same forecast for each 3 hour
                ...
                val initialWindowIndex = metadataList.indexOf(mediaItemToPlay)

                //exoPlayer.prepare(mediaSource)
                //TODO use queuenavigator
                try {
                exoPlayer.prepare(ConcatenatingMediaSource(mediaSource))
    ...
    override fun onPrepareFromSearch(query: String?, extras: Bundle?) {
        mediaSources[MUSIC]!!.whenReady {
            val metadataList = mediaSources[MUSIC]!!.search(query ?: "", extras ?: Bundle.EMPTY)
            if(metadataList.isEmpty()){
                val metadataList = mediaSources[METEO]!!.search(query ?: "", extras ?: Bundle.EMPTY)
            }
     ...
     */
    private fun buildPlaylist(item: MediaMetadataCompat, type: String): List<MediaMetadataCompat> =
            mediaSources[type]!!.filter { it.album == item.album }.sortedBy { it.trackNumber }
